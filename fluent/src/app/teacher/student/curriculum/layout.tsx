        ...
        <div className="w-[20vw] bg-gray-800 text-white p-4 flex flex-col space-y-2">
          ...
          >
            {Diaryopen ? "🔻" : "▶️"} Diary Notes
              ...
              {Diaryopen && (
              <>
                <Link href={`/teacher/student/curriculum/diary_note?user=${user}&type=teacher&id=${id}&student_name=${student_name}`}>
                    <span className={`block p-2 rounded ml-3 ${pathname === "/teacher/student/curriculum/diary_note" ? "bg-gray-700" : ""}`}>새로운 Diary 만들기</span>
                </Link>
                <nav className="">
                {Array.isArray(text2Data) && text2Data.length > 0 ? (
                text2Data.slice().reverse().map((item: any, index) => (
                    <Link key={index} href={`/teacher/student/curriculum/diary_note/text?user=${user}&type=teacher&id=${id}&student_name=${student_name}&item_id=${item._id}`}>
                    <span className={`ml-3 block p-2 rounded ${pathname === `/teacher/student/curriculum/diary_note/text?item_id=${item._id}` ? "bg-gray-700" : ""}`}>
                        {item.class_date}
                    </span>
                    </Link>   
                ))
